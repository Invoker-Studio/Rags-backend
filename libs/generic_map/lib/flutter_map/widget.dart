...
class FlutterMapViewState extends State<FlutterMapView> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return FlutterMap(
      ...
      options: MapOptions(
        ...
        initialCenter: widget.initialLocation?.latLng ?? const LatLng(50.5, 30.51),
        initialZoom: 16,
            ...
            : (event) async {
                if (event is MapEventMoveStart) {
                  ...
                  if (widget.centerMarker == null) {
                    if (kDebugMode) {
                      print('interactive but no center marker provided, skipping address retrieval');
                    }
                  ...
                  //   ),
                  // );
                  final markerBox = widget.markerKey.currentContext?.findRenderObject() as RenderBox;
                  ...
                  final place = await widget.addressResolver?.call(
                    widget.provider is MapBoxProvider ? MapProviderEnum.mapBox : MapProviderEnum.openStreetMaps,
                    latLng,
                  );
      ...
      children: [
        ...
        MarkerLayer(
          ...
          markers: widget.provider.parseMarkers(widget.markers).toList() as List<Marker>,
        ),
        if (widget.polylines.isNotEmpty)
          PolylineLayer(
            polylines: widget.provider.parsePolyLines(widget.polylines).toList() as List<Polyline>,
          ),
        CircleLayer(
          circles: widget.provider.parseCircleMarkers(widget.circleMarkers) as List<fmap.CircleMarker>,
        ),
